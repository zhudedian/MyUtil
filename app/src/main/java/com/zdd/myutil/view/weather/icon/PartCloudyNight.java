...
public class PartCloudyNight extends View {
    ...
    private float width,width1,height,height1,width2;
    private int paddingLeft,paddingRight,paddingTop,paddingBottom;
    public PartCloudyNight(Context context, int paintWidth) {

        this(context, null);
        this.paintWidth = paintWidth;
        mPaint.setStrokeWidth(paintWidth);
    }
    public PartCloudyNight(Context context) {
        this(context, null);
    }

    public PartCloudyNight(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PartCloudyNight(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
//        starPaint2.setStrokeJoin(Paint.Join.MITER);//锐角
    ...
    protected void onDraw(Canvas canvas) {
        ...
        path6.addCircle(radius5*90/100,radius5*93/100,radius5, Path.Direction.CCW);
        path7.addCircle(radius5*36/100,radius5*43/100,radius5, Path.Direction.CCW);
        path6.op(path7, Path.Op.DIFFERENCE);
        path6.op(path1, Path.Op.DIFFERENCE);
        canvas.drawPath(path6,mPaint);
        canvas.drawPath(path1,mPaint);

        drawStar(canvas,alpha1,width2*0.35f,0,width2*0.11f);
        drawStar(canvas,alpha2,width2*0.22f,width2*0.40f,width2*0.08f);
